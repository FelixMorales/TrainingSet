...
def ProductoPunto(layer):
    ...
    value = np.zeros(len(parent.objects[0].filters), dtype=object)
    bias = np.zeros(len(parent.objects[0].filters), dtype=object)
...
def logaritmo(layer):
    parent = layer.node.parents[0]
    layer.value = decimal.Decimal(np.log(float(parent.objects[0].value))*-1)
...
def c_filter_der(layer):
    ...
    filter_der = np.zeros(2, dtype=object)
    ...
    layer.value_der_total = np.zeros((layer.value_der.shape), dtype=object)

def b_filter_der(layer):
    kid = layer.node.kids[0]
    filter_der = np.zeros((layer.filters.shape), dtype=object)

    filter_der[0] = layer.value*kid.objects[0].value_der[0]
    filter_der[1] = layer.value*kid.objects[0].value_der[1]

    value_der = np.zeros((layer.value.shape), dtype=object)
    bias_der = np.zeros((layer.value.shape), dtype=object)
    ...
    layer.filter_der_total = np.zeros((layer.filter_der.shape), dtype=object)
    
    layer.value_der = value_der
    layer.value_der_total = np.zeros((layer.value_der.shape), dtype=object)

    layer.bias_der = bias_der
    layer.bias_der_total = np.zeros((layer.bias_der.shape), dtype=object)
...
def removeFilterNodeB(layerNodeB):
    if layerNodeB.filters[0] is not None and len(layerNodeB.filters[0]) > 0:
        ...
        auxFilter = np.zeros((2, newFilterShape[0]), dtype=object)
...
def deleteLastFilter(oldFilter, newShape):
    ...
    newFilter = np.zeros(tuple(newShape), dtype=object)
...
def createFilterA(networkObjects):
    ...
    filters = np.zeros((networkObjects[2],networkObjects[0], networkObjects[1], 3), dtype=object)
    for f in filters:
        for i in range(networkObjects[0]):
            for j in range(networkObjects[1]):
                f[i][j] = [decimal.Decimal(*np.random.rand(1)), 
                            decimal.Decimal(*np.random.rand(1)), 
                            decimal.Decimal(*np.random.rand(1))]       
    return filters

def createValueA(networkObjects):
    valueA = np.zeros((networkObjects[0], networkObjects[1], 3), dtype=object)
    
    for i in range(networkObjects[0]):
        for j in range(networkObjects[1]):
            valueA[i][j] = decimal.Decimal(*np.random.rand(1))

    return valueA

def createFilterB(networkObjects):

    filters = np.zeros((2, networkObjects[2]), dtype=object)

    for f in filters:
        for i in range(networkObjects[2]):
            f[i] = decimal.Decimal(*np.random.rand(1))
